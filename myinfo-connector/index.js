...
class MyInfoConnector {
  ...
  getMyInfoPersonData = async function (
    ...
    try {
      ...
      //create API call to exchange authcode for access_token
   ...
   * @param {string} privateSigningKey -  Your application private signing key in .pem format
   * @param redirectUrl - Redirect URL for web application
   ...
   * @param sessionPopKeyPair - Session PopKeyPair used to sign DPoP
   * @param {Array} privateEncryptionKeys -  Your application private encryption keys in .pem format, pass in a list of private keys that corresponds to JWKS encryption public keys
   * @param scope - Space separated list of attributes to be retrieved from Myinfo
   * @returns {Promise} Returns the Person Data (Payload decrypted + Signature validated)
   */
  ...
  callTokenAPI = async function (
    ...
    strParams =
      ...
      authCode +
      "&redirect_uri=" +
      redirectUrl + // redirect url should match redirect url used in /authorise call
      "&client_id=" +
   ...
   */
  callPersonAPI = async function (sub, accessToken, sessionEphemeralKeyPair, scope) {
    let urlLink;
    ...
    let strParams = "scope=" + encodeURIComponent(scope);
  ...
  getPersonDataWithToken = async function (
    ...
    try {
      console.log("getPersonDataWithToken accessToken: ", accessToken, "sessionEphemeralKeyPair: ", sessionEphemeralKeyPair, "privateEncryptionKeys: ", privateEncryptionKeys)
